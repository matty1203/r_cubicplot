...
## Logistic Regression Model: Magnetopause vs Other Events
...
From the matrix we can clearly see that the accuracy is about `r round(conf_mat$overall['Accuracy']*100,1)`% . Eventhough it gives better accuracy than the Model above with Dataset 3 but still the sensitivity of the Model is similar to that of  Logistic Regression Model which classifies Magnetopause Events by using the Dataset3.  `r round(conf_mat$byClass['Sensitivity']*100,1)`% of the Bow Shock boundaries were classified correctly. But when compared to Model that was trained with Dataset 3, The `NE` points were classified more accurately in this model.The specificity of this model is `r round(conf_mat$byClass['Specificity']*100,1)`% which means the `r round(conf_mat$byClass['Specificity']*100,1)`% of the NE Points were classified correctly through this model.
...
cassini.rfmodel <- ranger(type_cross ~ ., data = new_trainData)
new_testData$pred<-predict(cassini.rfmodel,data = new_testData)$predictions
confusionMatrix(new_testData$pred, new_testData$type_cross)
