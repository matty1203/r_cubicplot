...
```
```{r }
ggplot(data=temp, aes(x=TimeStamp)) +ylim(-3,3)+
  geom_line(aes(y = -BX_KRTP.nT.), color = "green")+
  geom_line(aes(y = -BY_KRTP.nT.), color = "blue")+
  geom_line(aes(y = BZ_KRTP.nT.), color = "red")+
  ylab("Bxyz(nT)")+geom_hline(aes(yintercept = 0), col = 8)+
